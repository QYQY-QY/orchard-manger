    ...
    <div v-if="currentDetailAccount">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="用户类型">{{
          currentDetailAccount.userType
        }}</el-descriptions-item>
        <el-descriptions-item label="账号状态">{{
          currentDetailAccount.accountStatus === "active" ? "已激活" : "已禁用"
        }}</el-descriptions-item>
        <el-descriptions-item label="实名信息">{{
          currentDetailAccount.name
        }}</el-descriptions-item>
        <el-descriptions-item label="用户名">{{
          currentDetailAccount.username
        }}</el-descriptions-item>
        <el-descriptions-item label="手机号">{{
          currentDetailAccount.phone
        }}</el-descriptions-item>
        <el-descriptions-item label="性别">{{
          currentDetailAccount.sex === "0"
            ? "女"
            : currentDetailAccount.sex === "1"
            ? "男"
            : "未知"
        }}</el-descriptions-item>
        <el-descriptions-item label="所属果园">{{
          currentDetailAccount.orchardName
        }}</el-descriptions-item>
        <el-descriptions-item label="权限等级">{{
          currentDetailAccount.isAdmin === 1
            ? "超级管理员"
            : currentDetailAccount.isAdmin === 2
            ? "管理员"
            : currentDetailAccount.isAdmin === 3
            ? "工作人员"
            : "普通用户"
        }}</el-descriptions-item>
      </el-descriptions>
