  ...
  <div class="notice-list-wrapper">
    <div class="table-card">
      ...
      >
        ...
        <el-table-column label="发布对象" min-width="120" align="center">
          <template #default="scope">
            <el-tag
              :type="
                scope.row.groupId === '0'
      ...
      <div
        v-if="!listLoading && noticeList.length === 0"
...
const getNoticeList = async () => {
  try {
    ...
    if (props.filterParams.title) {
      params.title = props.filterParams.title;
    }
    if (props.filterParams.isFinish !== "") {
      params.isFinish = Number(props.filterParams.isFinish); // 转换为数字
    }
    if (props.filterParams.empId) {
      params.empId = Number(props.filterParams.empId); // 转换为数字
    }
    if (props.filterParams.orchardId) {
      params.orchardId = Number(props.filterParams.orchardId); // 转换为数字
    }
...
const handleDelete = async (id) => {
  try {
    ...
    // 或者可能是：await axios.post('/api/announceInfo/delete', { id })
