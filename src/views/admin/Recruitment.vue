...
const fetchRecruitList = async (customParams = {}) => {
  try {
    const requestParams = {
      title: "",
      empId: "", // 可以留空，或者也传用户ID
      senderId: userStore.user?.id || 0, // ✅ 关键：把当前用户ID传给 senderId
      orchardId: userStore.user?.orchardId || 1,
      isFinish: undefined,
      isRead: undefined,
      ...customParams,
    };
    const res = await axios.post("/api/announceInfo/listPage", requestParams);
...
const handleViewDetail = (row) => {
  console.log("点击查看的数据:", row); // 看这里有没有 status
