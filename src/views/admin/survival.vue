<template>
  ...
  <div class="page-container">
    ...
    <div class="train-card" v-if="showTrainCard">
      ...
      <div class="train-actions" v-if="trainCompleted && chartImages.length > 0 && !isAnnotating">
        <el-button type="primary" @click="chartsDialogVisible = true">
          <el-icon><Picture /></el-icon> 查看训练图表
        </el-button>
        <el-button type="success" @click="compareDialogVisible = true">
          <el-icon><View /></el-icon> 对比效果
        </el-button>
      </div>
    </div>
    ...
    <div class="image-grid" v-else>
      ...
      <div
        v-for="image in currentDisplayImages"
        :key="image.id"
        class="image-card"
        :class="{ 'labeled-card': image.annotated }"
    ...
    >
      <el-form :model="annotationForm" label-width="80px">
        ...
        <el-form-item label="成活率">
          <el-slider
            v-model="annotationForm.survivalRate"
            :min="0"
            :max="100"
        ...
        <el-form-item label="标注内容">
          <el-input
            ...
            type="textarea"
            :rows="4"
            placeholder="请描述成活情况、影响因素等"
          />
        </el-form-item>
        <el-form-item label="影响因素">
          <el-select
            v-model="annotationForm.factors"
            multiple
            placeholder="请选择影响因素"
            style="width: 100%"
          >
    ...
    <!-- 标注日志详情弹窗 -->
    <el-dialog
      v-model="showAnnotationLogsDialog"
      title="标注日志详情"
      width="800px"
    >
      ...
      <div class="compare-container">
        <div class="compare-summary">
          <div class="summary-text">
            ...
            <span>本次训练基于 YOLOv8 模型，使用 {{ trainStats.totalData }} 张标注图片进行 {{ trainStats.totalEpochs }} 个 Epoch 训练（第 {{ trainingCount }} 次训练）</span>
          </div>
        </div>
        ...
        <div class="compare-conclusion">
          ...
          <div class="conclusion-text">
            <strong>训练效果总结：</strong>经过 {{ trainStats.totalEpochs }} 轮迭代训练（第 {{ trainingCount }} 次训练），模型在检测精度、分类准确率和推理速度三个维度均有显著提升。
            mAP@0.5 从 {{ beforeMetrics.mapValue }}% 提升至 {{ afterMetrics.mapValue }}%，提升了 {{ improvementMetrics.mapImprovement }}%；
