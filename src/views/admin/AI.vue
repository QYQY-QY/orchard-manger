      ...
      <div class="chat-content" ref="chatContent">
        ...
        >
          <div class="message-bubble">
            <!-- 如果是用户消息且包含图片，显示图片预览 -->
            <div
              v-if="item.type === 'user' && item.imageUrl"
              class="image-preview"
            ...
            >
              <img
                :src="item.imageUrl"
                alt="用户上传"
                @load="() => console.log('用户图片加载成功:', item.imageUrl)"
                @error="
                  (e) => console.error('用户图片加载失败:', item.imageUrl, e)
                "
              />
            </div>
            ...
            <!-- AI消息逐行显示 -->
            <div
              v-if="item.type === 'ai' && item.lines && item.lines.length > 0"
            >
              <div
            ...
            <!-- AI返回的图片 -->
            <div
              v-if="item.type === 'ai' && item.imageUrl"
...
onMounted(async () => {
  console.log("AI 助手页面已加载");

  // 安全检查：确保 route 和 route.query 存在
  if (!route || !route.query) {
    console.log("路由或查询参数不存在");
    return;
  }

  // 检查是否有自动发送的消息
  if (route.query.autoSend === "true" && route.query.message) {
    console.log("检测到自动发送消息，准备处理...");

    try {
      const message = decodeURIComponent(route.query.message);
      console.log("解码后的消息:", message);
      ...
      // 直接执行发送逻辑（不调用 handleSendMessage）
      ...
      if (router) {
        ...
        console.log("已清除 URL 参数");
      }
    } catch (error) {
      console.error("自动发送消息失败:", error);
      ElMessage.error("自动发送消息失败，请手动发送");
    }
  } else {
    console.log("没有自动发送的消息");
  }
});
// 格式化消息
...
const hexToImageUrl = (hexString) => {
  ...
  try {
    ...
    for (let i = 0; i < paddedHex.length; i += 2) {
      ...
      if (isNaN(byte)) {
        console.error(
          `在第 ${i} 位置解析16进制失败: ${paddedHex.substring(i, i + 2)}`
        );
        continue;
      }
      bytes.push(byte);
    ...
    if (response.data.code === 200) {
      ...
      if (type === "maturity" && data.ripe_stats) {
        ...
        if (stats.maturity_distribution) {
          aiContent += `未成熟期：${
            stats.maturity_distribution["未成熟期（膨果期/青果期）"] || 0
          } 个\n`;
          aiContent += `转色期：${
            stats.maturity_distribution["转色期（着色期）"] || 0
          } 个\n`;
          aiContent += `成熟期：${
            stats.maturity_distribution["成熟期（商品采收期）"] || 0
          } 个\n`;
        }
