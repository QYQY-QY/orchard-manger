...
const activeZone = computed(() => {
  return (
    zoneList.value.find((zone) => zone.id === activeAreaId.value) || {
      id: "",
      trees: [],
      number: "",
      name: "",
...
const handleZoneUpdate = async (action) => {
  switch (action.type) {
    case "add":
      ...
      break;
    case "edit":
      const editIndex = zoneList.value.findIndex(
        (zone) => zone.id === action.data.id
      );
      if (editIndex > -1) {
...
const getZoneList = async () => {
  try {
    ...
    if (response.data && response.data.code === 200) {
      ...
      ElMessage.error(
        `获取区域列表失败：${response.data?.msg || "后端返回非200状态码"}`
      );
      zoneList.value = [];
    }
  } catch (error) {
...
const handleTreeDelete = (treeId) => {
  try {
    ...
    if (zoneIndex > -1) {
      // 过滤删除指定果树，保留其他字段
      zoneList.value[zoneIndex].trees = zoneList.value[zoneIndex].trees.filter(
        (tree) => tree.id !== treeId
      );
      console.log("删除果树成功", {
        treeId,
