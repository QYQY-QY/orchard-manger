  ...
  <div class="zone-sidebar">
    ...
    >
      ...
      >
        ...
        <el-form-item label="果树品种" prop="type">
          <el-select v-model="zoneForm.type" placeholder="请选择果树品种">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmAddZone"> 确认添加 </el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <el-form-item label="果树品种" prop="type">
          <el-select v-model="editForm.type" placeholder="请选择果树品种">
            <el-option label="沙糖桔" value="沙糖桔" />
            <el-option label="沃柑" value="沃柑" />
            <el-option label="温州蜜柑" value="温州蜜柑" />
            <el-option label="红美人" value="红美人" />
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmEditZone">
          确认修改
        </el-button>
      </template>
    </el-dialog>
...
const getFarmerList = async (orchardId) => {
  ...
  try {
    ...
    if (response.data.code === 200) {
      // 筛选出isAdmin=3的农户
      farmerList.value = response.data.data.filter(
        (item) => item.isAdmin === 3
      );
  ...
  try {
    ...
    if (response.data && response.data.code === 200) {
      ...
      const updatedZone = {
        ...
        number:
          props.zoneList.find((z) => z.id === currentZone.id)?.number || "",
...
const resetAddZoneForm = () => {
  const currentOrchardId = userStore.user?.orchardId
    ? Number(userStore.user.orchardId)
    : "";
...
const confirmAddZone = async () => {
  try {
    ...
    const submitData = {
      ...
      createTime: formatBackendTime(), // 新增：创建时间（去掉时区）
      updateTime: formatBackendTime(), // 新增：更新时间（去掉时区）
...
watch(
  ...
  (newList) => {
    if (props.activeAreaId && newList.length) {
      const selectedZone = newList.find(
        (zone) => zone.id === props.activeAreaId
      );
      emitZoneInfo(selectedZone);
    }
  },
...
onMounted(() => {
  if (props.activeAreaId) {
    const selectedZone = props.zoneList.find(
      (zone) => zone.id === props.activeAreaId
    );
    emitZoneInfo(selectedZone);
    // 新增：获取农户列表
...
.zone-detail-content :deep(.el-descriptions__label) {
...
.zone-detail-content :deep(.el-descriptions__content) {
  padding-left: 20px !important;
  text-align: left !important;
}

.zone-detail-content :deep(.el-descriptions__cell) {
