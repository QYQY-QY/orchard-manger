  ...
  >
    <div class="account-card" v-if="currentAccount">
      ...
      <div class="avatar-section">
        <img
          :src="
            currentAccount.realNamePhoto ||
            currentAccount.avatar ||
            'https://picsum.photos/100/100?text=默认头像'
          "
          alt="实名照片/头像"
        ...
        <div class="sex-text mt-2">
          {{
            Number(currentAccount.sex) === 1
              ? "男"
              : Number(currentAccount.sex) === 0
              ? "女"
              : "未填写"
          }}
        </div>
      ...
      <div class="info-section">
        ...
        <div v-if="!isEditing">
          <div class="info-row">
            ...
            <span class="value">{{
              Number(currentAccount.isAdmin) === 3 ? "农户" : "临时工"
            }}</span>
          </div>
          <div class="info-row">
            <span class="label">所属果园：</span>
            <span class="value">{{
              currentAccount.orchardName || currentAccount.orchardId || "-"
            }}</span>
          ...
          <div class="info-row">
            ...
            <span class="value">{{
              formatIdCard(currentAccount.id_card) || "未实名"
            }}</span>
          </div>
          <div class="info-row">
            ...
            <span class="value">
              <el-tag
                :type="
                  Number(currentAccount.status) === 0 ? 'danger' : 'success'
                "
              >
          ...
          <div class="info-row">
            ...
            <span class="value">{{
              formatTime(currentAccount.createTime)
            }}</span>
            <span class="label ml-4">最近使用时间：</span>
            <span class="value">{{
              formatTime(currentAccount.lastUseTime) || "-"
            }}</span>
          </div>
          <div class="info-row">
            <span class="label">账号工作天数：</span>
            <span class="value"
              >{{ calcWorkDays(currentAccount.activateTime) }} 天</span
            >
          </div>
          ...
          <div class="real-name-form" v-if="!currentAccount.realName">
            ...
            >
              ...
              <el-form-item>
                <el-button type="primary" size="small" @click="submitRealName"
                  >提交实名</el-button
                >
              </el-form-item>
            </el-form>
        ...
        >
          ...
          <el-form-item
            label="身份证号"
            prop="id_card"
            v-if="!currentAccount.id_card"
          >
            <el-input
              v-model="editForm.id_card"
              placeholder="请输入身份证号（仅未实名账号可填）"
            />
          </el-form-item>
        </el-form>
      </div>
