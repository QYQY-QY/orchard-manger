      ...
      >
        ...
        >
          <template #default="scope">
            <el-tag
              :type="
                scope.row.accountStatus === 'active' ? 'success' : 'danger'
              "
            >
              {{ scope.row.accountStatus === "active" ? "已激活" : "已禁用" }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="实名信息" min-width="150" />
        <el-table-column label="操作" min-width="120" align="center">
          <template #default="scope">
            <span class="table-actions" @click="handleViewDetail(scope.row)"
              >查看详情</span
            >
          </template>
        </el-table-column>
    ...
    if (res.data.code === 200) {
      ...
      accountList.value = adminData.map((item) => ({
        ...
        userType:
          item.isAdmin === 1
            ...
            ? "工作人员"
            : "普通用户", // 根据 isAdmin 映射
        accountStatus: item.status === 1 ? "active" : "inactive", // status: 1=已激活
        name: item.name || "未填写", // 对应 name 字段
        username: item.username, // 可选：用户名
        phone: item.phone, // 可选：手机号
        sex: item.sex, // 可选：性别
        orchardName: item.orchardName, // 可选：果园名称
        isAdmin: item.isAdmin, // 可选：保留原始值
      }));
      ...
      if (filterParams.accountType) {
        ...
        accountList.value = accountList.value.filter(
          (item) => item.isAdmin === Number(filterParams.accountType)
        );
      }
      if (filterParams.accountStatus) {
        accountList.value = accountList.value.filter(
          (item) => item.accountStatus === filterParams.accountStatus
        );
      }
    } else {
